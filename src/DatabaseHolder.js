...
class DatabaseHolder{
	...
	static connect(connectionString, database){
		let dbConfig = require(join(dirname(require.main.filename), '/config/db.js'));
		if (typeof connectionString === "undefined"){
			connectionString = 'mongodb://' + this.getAuthentication(dbConfig.username, dbConfig.password) + dbConfig.host + ':' + dbConfig.port + '/';
		...
		return new Promise((resolve, reject) => {
			require('mongodb').MongoClient.connect(connectionString, {
				...
				if (!error){
					...
					this.db = this.dbConnection.db(typeof database !== 'undefined' ? database : dbConfig.database);
					for (let collectionName in this.collections){
						this.collections[collectionName].create(this.db).then((collection)=>{
							createdCollections++;
							if (createdCollections + failedCollectionCreation >= this.collectionCNT){
								resolve(true);
							}
						});
					}
